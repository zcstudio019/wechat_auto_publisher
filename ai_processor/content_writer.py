...
- 标题优先使用被拒原因型、额度卡点型、申请前检查型、真实案例型。
- 标题禁止使用“如何科学规划”“必知基础”“关键事项”“稳健发展”“一文读懂”“全面解析”。
- 正文控制在 1800-2500 字之间，最多 5 个 ## 小节。
- 开头先用 2-3 段打中老板痛点，再进入案例；必须写老板常见误区、银行不批的真实原因、本文要解决的问题。
- 正文必须有一个企业融资案例，可以匿名，但要写清企业类型、融资卡点、银行关注点和调整方向。
- 每个小节尽量按“问题是什么 → 为什么影响审批 → 老板应该怎么做”展开。
- 必须拆解 3-5 个问题：征信、流水、负债、纳税、用途、担保、查询次数等。
- 必须给出 3-5 个解决建议：提前体检、资料梳理、控制查询、优化流水、匹配银行、规划续贷等。
- 金句卡最多 1 次，不能输出单独英文引号。
- 必须有风险提醒：不承诺放款、不夸大额度，强调根据企业实际情况评估。
- 结尾必须固定加入“企业融资体检”模块：适合人群、体检结果、行动引导（扫码做融资体检/添加顾问咨询）。
"""
...
- 优先使用被拒原因型、额度卡点型、申请前检查型、真实案例型
- 禁止使用“如何科学规划”“必知基础”“关键事项”“稳健发展”
- 可以自然保留企业贷、经营贷、融资、银行贷款、征信、批款、放款、利率等业务词
- 禁止写成「贷款融资专业解决方案服务」「上海企业经营贷融资贷款批款服务」这类堆词标题
- 示例：「银行拒贷后，老板先查这3点」「经营贷额度低，通常卡在这4处」「申请经营贷前，先做一次融资体检」「一家贸易公司被拒后，额度怎么做起来？」
"""
    if category == 'brand':
...
def _template_write_structured(topic, structure_list, pain_point, solution,
    ...
    if CONTENT_GROWTH_ENABLED:
        content = f"""很多老板一听到{topic}，第一反应是先问利率、问额度、问哪家银行更好批。但真正影响结果的，往往不是你问了哪家银行，而是银行看到你的流水、征信、负债和用途材料后，能不能判断这笔钱还得上。

常见误区是：被拒以后马上换银行，或者同时投好几家。老板以为是在增加机会，银行看到的可能是短期查询变多、资金需求更急、风险信号更明显。越急，越容易把下一次申请也做坏。

这篇文章不讲空泛大道理，只解决一个问题：老板做{topic}前，到底应该先查哪些被拒原因、哪些额度卡点，怎么把融资申请变成可评估、可优化的事。

## 真实案例：额度为什么没批上来

一家做贸易的小微企业，订单和流水都不算差，老板想用经营贷覆盖一批采购款。第一次申请时，银行只给了很低额度，原因不是单纯“银行太严”，而是企业流水波动大、回款有一部分走个人账户，法人近 3 个月又查了多次征信。

问题是什么？老板觉得自己有业务、有客户、有回款，银行应该认可。但银行审批看的不是一句“生意不错”，而是合同、发票、纳税、流水和还款来源能不能互相印证。

为什么影响审批？当银行看到流水断档、查询偏多、用途说明不清，会认为企业短期资金压力正在放大。额度自然会被压低，甚至直接被拒。

老板应该怎么做？先把材料重新排一遍：对公流水、主要合同、开票记录、纳税记录、回款节奏、现有负债，放在同一张表里看，再判断是补材料、调额度，还是换更匹配的产品。

## 银行不批，通常不是一个原因

第一看征信。不是只有逾期才算问题，短期查询次数、担保责任、信用卡使用率、其他贷款余额，都会影响银行对老板资金紧张程度的判断。

第二看流水。流水不是越大越好，关键是稳定、真实、能解释。突然进出的大额资金，如果和合同、发票、纳税对不上，反而会让审批更谨慎。

第三看负债。企业贷款、法人个人贷款、对外担保叠加后，银行会重新计算月供压力。老板自己觉得能扛住，不代表银行的模型会认可。

第四看用途。经营贷要能说清楚钱用在哪里，是采购、工资、租金、设备，还是项目垫资。用途越模糊，审批越容易卡。

## 申请前，老板先做这 5 个动作

① 先查企业和法人征信，确认逾期、查询、担保、负债有没有硬伤。

② 梳理近 6-12 个月流水，看是否能和合同、发票、纳税、回款节奏对应。

③ 算清现有月供和新增月供，别只看最高额度，要看实际还款压力。

④ 明确资金用途，把采购、工资、租金、设备或项目周转材料提前准备好。

⑤ 如果是续贷，至少提前 60-90 天做风险排查，不要等到到期前才补救。

## 风险提醒：别用包装代替优化

融资优化不是包装材料，也不是承诺一定能批。任何让你虚构流水、虚假合同、先交高额费用、承诺固定额度和固定利率的做法，都要谨慎。

真正有价值的做法，是先把审批逻辑讲清楚：哪些条件已经够，哪些地方会被卡，哪些动作能提高通过率，哪些情况应该暂缓申请。
